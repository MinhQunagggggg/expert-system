...
public class productDAO {
    ...
    public void ProductDelete(String product_id) {
        ...
        String sql2 = "delete from bill_detail where product_id=?";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(sql2);
            ps.setString(1, product_id);
    ...
    public List<Product> getTrendProduct() {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT TOP 10 p.product_id, p.product_name, p.product_price, p.product_describe, p.quantity, p.img "
                + "FROM product p "
                + "INNER JOIN bill_detail bd ON p.product_id = bd.product_id "
                + "GROUP BY p.product_id, p.product_name, p.product_price, p.product_describe, p.quantity, p.img "
