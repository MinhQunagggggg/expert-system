  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        if (user != null && user.getIsAdmin().equalsIgnoreCase("true")) {
            if (action == null) {
                List<Bill> bills = dao.getBillInfo();
                ...
                if (bill_id != null) {
                    ...
                    List<BillDetail> details = dao.getDetail(id);
                    request.setAttribute("detail", details);
                    request.getRequestDispatcher("admin/orderdetail.jsp").forward(request, response);
                } else {
                    response.sendRedirect("404.jsp"); // Handle missing bill_id
                }
            } else {
                response.sendRedirect("404.jsp"); // Handle unknown action
            }
        } else {
